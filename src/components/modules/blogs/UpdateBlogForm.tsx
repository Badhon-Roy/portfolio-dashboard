...
const UpdateBlogForm = ({blog} : {blog : IBlog}) => {
    ...
    const {
        ...
        defaultValues: {
            title: blog.title,
            tags: blog.tags.join(", "), // Convert the tags array to a comma-separated string
    ...
    const onSubmit: SubmitHandler<FieldValues> = async (data) => {
        ...
        try {
            const res = await updateBlog(blog._id, formattedData);
            if (res.success) {
                toast.success(res?.message, { id: toastLoading });
                reset();
                router.push('/dashboard/blogs/manage')
            } else {
    ...
    return (
        ...
        <div className="w-full p-6 bg-white/10 backdrop-blur-lg rounded-xl shadow-lg border-2 border-white">
            <h2 className="text-3xl font-bold text-center mb-5">Update Blog Post</h2>
