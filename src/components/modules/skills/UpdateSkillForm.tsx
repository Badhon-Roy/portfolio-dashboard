...
const skillSchema = z.object({
    ...
    experience: z.string().min(1, "Experience must be at least 1 character"),
...
const UpdateSkillForm = ({ skill }: { skill: ISkill }) => {
    ...
    const onSubmit: SubmitHandler<FieldValues> = async (data) => {
        ...
        try {
            const res = await updateSkill(skill?._id, formattedData);
            if (res.success) {
                toast.success(res?.message, { id: toastLoading });
                reset();
                router.push('/dashboard/skills/manage')
            } else {
    ...
    return (
        <div className="flex items-center justify-center">
            <div className="w-full p-6 bg-white/10 backdrop-blur-lg rounded-xl shadow-lg border-2 border-white">
                <h2 className="text-3xl font-bold text-center mb-5">Update Skill</h2>
